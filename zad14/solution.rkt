...
(provide (struct-out const)  (struct-out binop)  (struct-out var-expr)  (struct-out let-expr)  (struct-out if-expr) (struct-out cons-expr) (struct-out car-expr) (struct-out cdr-expr) (struct-out null-expr) (struct-out null?-expr) (struct-out app) (struct-out lam) (struct-out apply) expr? parse eval value? parse-app parse-lam)
...
(struct apply      (lam list) #:transparent)
...
(define (expr? e)
  (match e
    ...
    [(apply lam list) (and (expr? lam) (or (list? list) (expr? list)))]
    [_ false]))
...
(define (parse q)
  (cond
    ...
    [(and (list? q) (eq? (first q) 'list)) ;; dodano parsowanie list
     (parse-list (cdr q))]
    ...
    [(and (list? q) (eq? (length q) 4) (eq? (first q) 'if))
     (if-expr (parse (second q))
              (parse (third q))
              (parse (fourth q)))]
    [(and (eq? (first q) 'apply) (list? q) (eq? (length q) 3))
...
(define (parse-list q)
  (if (null? q)
      q
      (cons (parse (car q)) (parse-list (cdr q)))))
  ...
  (match e
    ...
    [(apply lam list)
     (cond
       [(list? list)
         (let* ([x (car list)]
                [y (if (const? x) x (if (number? x) (const x) x))]
                [e (const (eval-env y env))])
           (if (eq? 1 (length list))
               (eval-env (app lam e) env)
               (eval-env (apply (app lam e) (cdr list)) env)))]
       [(cons-expr? list)
         (let ([e (const (eval-env (cons-expr-e1 list) env))])
           (if (null-expr? (cons-expr-e2 list))
               (eval-env (app lam e) env)
               (eval-env (apply (app lam e) (cons-expr-e2 list)) env)))]
       [else (eval-env (apply lam (eval-env list env)) env)])]))
...
(define a (parse '(apply (lambda (x y) (+ x y)) (if true (list 1 2) (list 3 4)))))
a
(eval a)
;;(eval (apply (lam 'y (lam 'z (binop '+ (var-expr 'y) (var-expr 'z)))) (list (const 1) (const 2))))
;;(eval (apply (lam 'x (lam 'y (binop '+ (var-expr 'x) (var-expr 'y)))) (cons-expr (binop '+ (const 1) (const 1)) (cons-expr (const 2) (null-expr)))))