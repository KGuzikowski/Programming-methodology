...
(define (eval-env e env)
  (match e
    ...
    [(var-expr x) (eval-env (env-lookup x env) env)]
    [(if-expr eb et ef) (if (eval-env eb env)
                            (eval-env et env)
                            (eval-env ef env))]
    [(cons-expr e1 e2) (cons e1 e2)]
    [(car-expr e) (eval-env (cons-expr-e1 e) env)]
    [(cdr-expr e) (eval-env (cons-expr-e2 e) env)]
    [(null-expr) null]
...
;(define a (parse '((lambda (x) (+ 3 3))(/ 5 0))))
;a
;(eval a)
;(define b (parse '(let [if-fun (lambda (b t e) (if b t e))]
;                (if-fun true 4 (/ 5 0)))))
;b
;(eval b)
;(define c (parse '(car (cons (+ 2 2) (/ 5 0)))))
;c
;(eval c)
